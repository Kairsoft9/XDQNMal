...
// 定义要 hook 的常见反射方法
function hookReflectionMethods() {
    var reflectionMethods = ["getName", "getMethod", "getMethods", "forName", "invoke", "getDeclaredMethod", "getDeclaredMethods"];
    reflectionMethods.forEach(function (method) {
        var ReflectClass = Java.use("java.lang.reflect." + method.charAt(0).toUpperCase() + method.slice(1));
        ReflectClass[method].overload().implementation = function () {
            console.log("Reflection method " + method + "() is invoked!");
...
function hookPermission() {
    Java.perform(function () {
        ...
        var requestPermissions =ActivityCompat.requestPermissions.overload('android.app.Activity', '[Ljava.lang.String;', 'int')
        requestPermissions.implementation = function (activity, permissions, requestCode) {
            for (var i = 0; i < permissions.length; i++) {
                log(permissions[i]);
            }
            return requestPermissions.call(this, activity, permissions, requestCode);
        };
        var ContextCompat = Java.use("androidx.core.content.ContextCompat");
        var checkSelfPermission = ContextCompat.checkSelfPermission.overload('android.content.Context', 'java.lang.String');
        checkSelfPermission.implementation = function (context, permission) {
            log(permission);
            return checkSelfPermission.call(this, context, permission);
        };
        var PackageManager = Java.use("android.content.pm.PackageManager");
        var checkPermission = PackageManager.checkPermission.overload('java.lang.String', 'java.lang.String');
        checkPermission.implementation = function (permissionName, packageName) {
            log(permissionName);
            return checkPermission.call(this, permissionName, packageName);
        };
    });
}
